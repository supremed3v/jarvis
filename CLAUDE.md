...
JARVIS is a local-first personal AI operating system, under active implementation following its spec library. Phases 1-3 (Foundation, Runtime, Task Execution: SPEC-0001 through SPEC-0017) are complete. Phase 4 Intelligence is complete in full: the Agent layer (SPEC-0018-0025: Agent Interface, Manifest System, Registry, Lifecycle Manager, Execution Loop, Context Builder, Permission Model, Communication Protocol), LLM layer (SPEC-0026-0033: Provider Interface, Ollama Integration, Model Config/Router, Prompt Templates, Streaming Response Handler, Context Window Manager, Token Budget Manager), Memory layer (SPEC-0034-0042), and Tools layer (SPEC-0043-0052) are all `Completed`. Phase 5 Applications' Voice branch is also complete through SPEC-0060 (Voice Session Manager). 60 of 182 specs are `Completed` per `docs/agents/JARVIS_BUILD_TRACKER.md`, which is the authoritative source for what's actually built — check it (or `context/features/FEATURE_INDEX.md`, generated from it) before assuming a spec is unimplemented or before trusting this paragraph's counts, which are a snapshot and will lag behind the tracker as work continues.

The Go workspace (`go.work`, Go 1.24) currently has five real modules with code, tests, and (for `services/core`) one third-party dependency (`gopkg.in/yaml.v3`, since SPEC-0019):
- `packages/config`, `packages/errors`, `packages/logger`, `packages/shared-types` (Foundation layer)
- `services/core` (Runtime, Task Execution, Agent, LLM, Memory, Tools, and Voice layers so far — e.g. `runtime.go`, `eventbus.go`, `task_*.go`, `agent*.go`, `llm_provider.go`, `stream_handler.go`, `memory_*.go`, `tool_*.go`, and the `services/core/voice` subpackage)

`agents/core-agent`, `agents/developer-agent`, `agents/research-agent`, `apps/desktop`, `services/memory`, `services/tools`, and `services/voice` (the top-level scaffold directories, distinct from the real implementations living inside `services/core`) are still empty scaffold directories (`.gitkeep` only) — no spec has required a concrete implementation in any of them yet.

Build/vet/test commands now exist: use `scripts/go_all.ps1` (see below) rather than `go build ./...` from the repo root, which fails for workspace-structural reasons unrelated to whether code exists. The next unit of work per `docs/execution/JARVIS_IMPLEMENTATION_ORDER.md` is SPEC-0061 (Voice Streaming Pipeline), continuing Phase 5's Voice branch. Do not invent build tooling that isn't specified; implement exactly what the relevant SPEC describes.
